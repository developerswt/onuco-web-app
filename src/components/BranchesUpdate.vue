  ...
  <div class="container">
    ...
    <div style="padding: 20px;">
      ...
      <div class="example-wrapper">
        ...
        <div style="height: 100%;">
          <ag-grid-vue :domLayout="domLayout" class="ag-theme-alpine" :columnDefs="columnDefs" :rowData="rowData"
            :editType="editType" :rowSelection="rowSelection" :defaultColDef="defaultColDef" :suppressExcelExport="true"
            :popupParent="popupParent" @grid-ready="onGridReady" @cell-value-changed="onCellValueChanged"
            @row-clicked='onCellClicked' cacheQuickFilter=true :pagination="true" :paginationPageSize="paginationPageSize"
            isLoding="true">
          </ag-grid-vue>
        </div>
      </div>
    ...
    <div v-if="showChildRow">
      <div class="modal fade show" tabindex="-1" aria-labelledby="exampleModalLabel"
        style="display:block;position: fixed; top: 170px;left: 500px;" aria-modal="true" role="dialog">
...
export default {
  ...
  methods: {
    ...
    async update(id) {
      ...
      try {
        const res = await axiosInstance.put(`/Branches` + '?' + 'id=' + id + '&name=' + this.childPara.name + '&desc=' + this.childPara.description);
        console.log(res);
        this.ismodel = true;
