...
export default {
    ...
    methods: {
        ...
        hasCompleteWatchTime(subjectId) {
            if (!Array.isArray(this.watchTimeDatas)) {
                return false;
            }

            // Check if any watch time data for the given subjectId has a WatchTime of 100
            return this.watchTimeDatas.every(video => {
                if (!Array.isArray(video.watchTimeData)) {
                    return false;
                }

                const watchData = video.watchTimeData.find(watch => watch.videoId === subjectId);
    
                // Parse watchTime to a number and check if it's equal to 100
                return watchData && parseFloat(watchData.watchTime) === 100;
                console.log(watchData && parseFloat(watchData.watchTime) === 100);
            });
        },
...
isProgressBarComplete(videoId) {
    return Array.isArray(this.watchTimeDatas) && this.watchTimeDatas.some(video =>
        Array.isArray(video.watchTimeData) && video.watchTimeData.some(
            watch => watch.videoId === videoId && parseFloat(watch.watchTime) === 100
        )
    );
},
...
isProgressBarEmpty(videoId) {
    return !Array.isArray(this.watchTimeDatas) || !this.watchTimeDatas.some(video =>
        Array.isArray(video.watchTimeData) && video.watchTimeData.some(
            watch => watch.videoId === videoId && parseFloat(watch.watchTime) > 0
        )
    );
},
    ...
    async created(){
        ...
        try {
            ...
            this.book = res.data; 
            const result = await AxiosInstance.get('/StateManagement/1');
